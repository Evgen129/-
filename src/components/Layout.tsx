...
export function Layout() {
  ...
  const navItems = [
    ...
    { to: '/solutions', icon: Lightbulb, label: 'Решения' },
    ...(isAdmin ? [
  ...
  return (
    <div className="flex h-screen w-full flex-col bg-slate-900 text-slate-100">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around border-t border-slate-800 bg-slate-900 pb-safe pt-1 sm:static sm:h-auto sm:justify-start sm:gap-6 sm:border-t-0 sm:px-6 sm:py-4">
        {navItems.map(({ to, icon: Icon, label }) => (
          <NavLink
            key={to}
            to={to}
            className={({ isActive }) => cn(
              "flex flex-col items-center p-2 text-xs font-medium transition-colors sm:flex-row sm:gap-2 sm:text-sm",
